...
The core operator that handles the cube cut workflow.
"""
...
class MESH_OT_cube_cut(bpy.types.Operator):
    ...
    def _update_first_vertex_preview(self, context, event):
        ...
        if self._is_2d_view:
            snapped, plane_normal = snapping.calculate_first_vertex_snap_2d(context, event)
            ...
            snapped, face_normal, obj, was_clamped = snapping.calculate_first_vertex_snap_3d(context, event)
            if snapped is not None:
    ...
    def _update_depth_preview(self, context, event):
        ...
        if self._is_2d_view:
            depth = snapping.calculate_depth_from_mouse_2d(
                context, event, self._depth_start_mouse_pos[0]
            )
        else:
            depth = snapping.calculate_depth_from_mouse_3d(
                context, event,
    ...
    def _confirm_first_vertex(self, context, event):
        ...
        if self._is_2d_view:
            ...
            # 3D view
            snapped, face_normal, obj, _was_clamped = snapping.calculate_first_vertex_snap_3d(context, event)
            if snapped is None:
    ...
    def _confirm_depth(self, context, event):
        """Confirm the depth and execute the cut."""
        success, message = geometry.execute_cube_cut(
            context,
            self._first_vertex,
            self._second_vertex,
            self._depth,
    ...
    def _update_header(self, context):
        ...
        if self._state == self.STATE_FIRST_VERTEX:
            text = "Cube Cut: Click to set first corner | ESC to cancel"
        elif self._state == self.STATE_SECOND_VERTEX:
            text = "Cube Cut: Click to set opposite corner | ESC to cancel"
        elif self._state == self.STATE_DEPTH:
            text = f"Cube Cut: Move mouse to set depth ({self._depth:.3f}) | Click to confirm | ESC to cancel"
        else:
            text = "Cube Cut | ESC to cancel"
